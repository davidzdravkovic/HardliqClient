...
const PILE_CONFIG = [
  { key: 'pending', countKey: 'pending', label: 'Scheduled', pileClass: 'stats-pile-pending', stackClass: 'stats-stack-pending' },
...
export default function WorkspaceStats({ topicId, refreshKey }) {
  ...
  return (
    <section className="stats-card workspace-stats" aria-label="Task statistics">
      ...
      {stats.totalTasks === 0 ? (
        <p className="workspace-stats-empty">No tasks yet.</p>
      ) : (
        <>
          <div
          ...
          <ul className="stats-piles" aria-label={stackLabel || 'Task breakdown'}>
            {piles.map((pile) => (
